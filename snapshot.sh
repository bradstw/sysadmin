#!/bin/bash
# update path info to match results of echo $PATH so it will run in cron
...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		# Get the attched volumes Name from the tags to set the snapshots name
		tag_name=$(aws ec2 describe-volumes --region $region --output=text --volume-ids $volume_id --query 'Volumes[0].{Devices:Tags[0].Value}')
		...
		# Add a "CreatedBy:AutomatedBackup" and Name tag to the resulting snapshot.
		# Why? Because we only want to purge snapshots taken by the script later, and not delete snapshots manually taken.
		aws ec2 create-tags --region $region --resource $snapshot_id --tags Key=CreatedBy,Value=AutomatedBackup Key=Name,Value=$tag_name 
	done
}
